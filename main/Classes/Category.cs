namespace TaskbarGroups.Classes
{
    ...
    public class Category
    {
        ...
        private readonly Regex specialCharRegex = new Regex("[*'\",_&#^@]");
        ...
        public void CreateConfig(Image groupImage)
        {
            ...
            ShellLink.InstallShortcut(Path.GetFullPath(AppDomain.CurrentDomain.FriendlyName),
                "TaskbarGroup.menu." + Name, path + " shortcut", Path.GetFullPath(path),
                Path.GetFullPath(path + @"\GroupIcon.ico"), path + "\\" + Name + ".lnk", Name);

            // Build the icon cache
            CacheIcons();

            File.Move(path + "\\" + Name + ".lnk",
                Path.GetFullPath(@"Shortcuts\" + Regex.Replace(Name, @"(_)+", " ") +
                                 ".lnk")); // Move .lnk to correct directory
        }
        ...
        public Image LoadImageCache(ProgramShortcut shortcutObject)
        {
            ...
            if (File.Exists(programPath) || Directory.Exists(programPath) || shortcutObject.IsWindowsApp)
                try
                {
                    ...
                    var cacheImagePath = Path.GetDirectoryName(Application.ExecutablePath) + @"\config\" + Name +
                                         @"\Icons\" + (shortcutObject.IsWindowsApp
                                             ? specialCharRegex.Replace(programPath, string.Empty)
                                             : Path.GetFileNameWithoutExtension(programPath)) +
                                         (Directory.Exists(programPath)
                                             ? "_FolderObjTSKGRoup.jpg"
                                             : ".png");
                ...
                catch (Exception)
                {
                    ...
                    var path = MainPath.path + @"\config\" + Name + @"\Icons\" +
                               Path.GetFileNameWithoutExtension(programPath) + (Directory.Exists(programPath)
                                   ? "_FolderObjTSKGRoup.png"
                                   : ".png");
